...
export async function markMessagesDeliveredService(messageIds: string[]): Promise<BaseResponse<{ success: boolean }>> { return markDelivered(messageIds) }
export async function getUnreadCountService(chatId: Id, userId: Id): Promise<BaseResponse<{ unreadCount: number }>> { return unreadCount(chatId, userId) }
...
export async function uploadChatAttachmentService(chatId: Id, files: File[]): Promise<BaseResponse<{ urls: string[] }>> {
    const formData = new FormData();
    files.forEach((f) => formData.append('files', f));
    return uploadChatAttachment(chatId, formData);
}
