...
function getRoutingTypeText(routingType: RoutingType | null): string {
    switch (routingType) {
...
function getRoutingTypeColor(routingType: RoutingType | null): ThemeColors {
    switch (routingType) {
        case RoutingType.SERVICE:
            return 'secondary100';
        case RoutingType.PRODUCT:
...
function RouteItemComponent({ route, onPress }: RouteItemProps) {
    ...
    const routingTypeText = getRoutingTypeText(route.routingType);
    const routingTypeColor = getRoutingTypeColor(route.routingType);
    ...
    return (
        ...
        >
            <Box flexDirection="row" justifyContent="space-between" alignItems="center" mb="y12" gap="x8">
                ...
                <Box backgroundColor="primary100" px="x16" py="y4" borderRadius="s20" style={{ flexShrink: 0 }}>
            ...
            <Box flexDirection="row" alignItems="center" justifyContent="space-between" mt="y12">
                <Text preset="text13" color={statusColor}>
                    {statusText}
                </Text>
                <Box backgroundColor={routingTypeColor} px="x12" py="y4" borderRadius="s12">
