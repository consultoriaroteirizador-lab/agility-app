...
// Re-exportar tipos
export type { LocationData, TrackingAuthConfig } from './backgroundLocationService';
...
export async function startLocationTracking(
  driverId: string,
  updateDriver?: (variables: { id: string; payload: { currentLatitude: number; currentLongitude: number; isAvailable: boolean } }) => void,
...
export async function stopLocationTracking(
  driverId?: string,
  updateDriver?: (variables: { id: string; payload: { isAvailable: boolean } }) => void
): Promise<void> {
  if (!legacyTrackingActive) {
    return;
  }
