...
import { create, findOne, findByUser, getMessages, sendMessage, close, markRead, markDelivered, unreadCount, createDriverSupport, createDriverCustomer, createCustomerSupport, type ChatItem, type MessageItem } from './chatAPI'
...
export async function createDriverSupportChatService(params: { driverId?: string; supportId?: string }): Promise<BaseResponse<ChatItem>> { return createDriverSupport(params) }
...
export async function uploadChatAttachmentService(files: File[]): Promise<BaseResponse<{ urls: string[] }>> {
    ...
    return (await import('./chatAPI')).uploadChatAttachment(formData);
}
